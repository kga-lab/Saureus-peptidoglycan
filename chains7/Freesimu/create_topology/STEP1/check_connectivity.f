...
!STEP-A calculate jump=> fjp and sum=> sum1 and decide to close(fp) for GLY
          ...
          if(trim(resnam(kr,ic)).ne.'NAG')then !NAM->NAG; NAM->ALA; AA->AA
             ...
             if(jj.eq.-2) then
               ...
               enddo
             elseif((jj.eq.-4).and.
     +            ((trim(bondfile(kr,ic)).eq.'bonds_LYS_linked.dat').or.
     +             (trim(bondfile(kr,ic)).eq.'bonds_LYS_bridge.dat')))
     +              then
                    fjp = 6
           ...
           elseif(jj.eq.-10) then      ! resnam = NAG and pbc linked
                 read(88,*) i1,jc  !these are chain numbers, and in order.
               ...
               if(jc.eq.ic)then
                 ...
                 sum1 = sum(Nar(kr:,ic)) +sum(Nar(:,ic+1:jc-1))
               endif
               do while(flag2.eq.0)
                  fjp = fjp + 1
                 if(bondfile(fjp,jc).eq.'bonds_RightNAM_linked.dat')
     +              then 
                    flag2=1
                 endif
               enddo
                 sum1=sum1+sum(Nar(1:fjp-1,jc))

           else               !resnam = NAG normal     
                fjp=1
                sum1=Nar(kr,ic)
           endif
!===================================
!STEP-B to get a pair of bonded particles 
           if(mod(jj,2).eq.0) then
              ...
              elseif(jj.eq.-10)then
                  open(77,file=trim(adjustl(pathi))//bondfile(fjp,jc))
                  read(77,*) comment  
                  do ii=1,Nbr(fjp,jc)
                   read(77,*) i1,j1
                   if(j1.eq.jj+1)then
